...
if [ $# -lt 3 ]; then
    echo "Usage ./run_3_train.sh variant train_script data_path [train_options]"
    exit 1
fi
...
python -u ${TRAIN_SCRIPT} ${DATA_PATH}/in_data.pkl 2 --model_name ${DATA_PATH}/in_model --direction "INGRESS" ${@:4}
python -u ${TRAIN_SCRIPT} ${DATA_PATH}/out_data.pkl 2 --model_name ${DATA_PATH}/out_model --direction "EGRESS" ${@:4}
